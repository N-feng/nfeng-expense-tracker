...
export const createOrUpdateTranscation = async (
  ...
  try {
    ...
    const transactionsRef = id
      ? doc(firestore, "transactions", id)
      : doc(collection(firestore, "transactions"));

    await setDoc(transactionsRef, transactionData, { merge: true });

    return {
      success: true,
      data: { ...transactionData, id: transactionsRef.id },
    };
  } catch (err: any) {
